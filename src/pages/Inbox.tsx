...
export default function Inbox() {
  ...
  const add = () => {
    if (!text.trim()) return;
    setData(current => ({ ...current, inbox: [{ id: nextId(current.inbox), text: text.trim(), created: new Date().toISOString() }, ...current.inbox] }));
    setText("");
  };
          ...
          >
            ...
            {active === item.id && (
              <div className="border-t border-[var(--border)] px-4 py-3 bg-[var(--secondary)] rounded-b-xl">
                <p className="text-[11px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)] mb-2">Converter em</p>

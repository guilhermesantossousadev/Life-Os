  ...
  return (
    <div className="p-6" style={{ fontFamily: "var(--font-ui)" }}>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { const name = prompt("Nome do patrimônio:"); if (!name) return; const rawValue = prompt("Valor estimado (R$):", "0"); setData(current => ({ ...current, assets: [...current.assets, { id: nextId(current.assets), type: "Eletrônico", name, details: { modelo: name, compra: today(), valor: Number(rawValue) || 0, serie: "" }, warranty: today(), value: Number(rawValue) || 0 }] })); }} className="flex items-center gap-1.5 px-3 py-1.5 text-[13px] bg-[var(--primary)] text-white rounded-lg"><Plus size={14} /> Novo item</button>
      </div>

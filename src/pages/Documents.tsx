...
import { FileText, Search, Upload, Download, Tag, Filter, Trash2 } from "lucide-react";
...
export default function Documents() {
  ...
  }).sort((a, b) => newestFirst ? b.updated.localeCompare(a.updated) : a.name.localeCompare(b.name));

  const upload = async (files: FileList | null) => {
    if (!files) return;
    for (const file of Array.from(files)) {
      if (file.size > 2_000_000) { alert(`${file.name}: o limite local é 2 MB.`); continue; }
      const dataUrl = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = reject; reader.readAsDataURL(file); });
      setData(current => ({ ...current, documents: [{ id: nextId(current.documents), name: file.name.replace(/\.[^.]+$/, ""), category: activeCategory === "Todos" ? "Outros" : activeCategory, tags: [], size: `${(file.size / 1024 / 1024).toFixed(1)} MB`, updated: today(), type: file.type.includes("pdf") ? "pdf" : "arquivo", dataUrl }, ...current.documents] }));
    }
  };

  const download = (document: (typeof data.documents)[number]) => {
    const anchor = window.document.createElement("a");
    anchor.href = document.dataUrl || URL.createObjectURL(new Blob([`Documento: ${document.name}\nCategoria: ${document.category}`], { type: "text/plain" }));
    anchor.download = document.dataUrl ? document.name : `${document.name}.txt`;
    anchor.click();
  };
  ...
  return (
    <div className="p-6" style={{ fontFamily: "var(--font-ui)" }}>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => inputRef.current?.click()} className="flex items-center gap-1.5 px-3 py-1.5 text-[13px] font-medium rounded-lg bg-[var(--primary)] text-white hover:bg-blue-700">
      ...
      <div className="flex items-center gap-3 mb-5 max-w-2xl">
        ...
        <button onClick={() => setNewestFirst(value => !value)} className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-[var(--border)] bg-white text-[13px] text-[var(--muted-foreground)] hover:bg-[var(--secondary)]">
            ...
            return (
              ...
              >
                ...
                <div className="mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => download(doc)} className="w-full flex items-center justify-center gap-1 py-1.5 rounded-lg bg-[var(--secondary)] text-[11px] text-[var(--muted-foreground)] hover:bg-[var(--accent)] hover:text-[var(--accent-foreground)] transition-colors">
                    <Download size={10} /> Baixar
                  </button>
                  <button aria-label="Excluir documento" onClick={() => setData(current => ({ ...current, documents: current.documents.filter(item => item.id !== doc.id) }))} className="w-full mt-1 py-1 text-[10px] text-red-500 hover:underline"><Trash2 size={9} className="inline mr-1" />Excluir</button>
                </div>
              </div>
            );
          })}

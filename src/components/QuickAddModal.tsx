...
const options = [
  ...
  { id: "expense" as const, label: "Despesa", icon: TrendingDown, color: "#EF4444" },
  { id: "income" as const, label: "Receita", icon: TrendingUp, color: "#059669" },
  { id: "note" as const, label: "Nota", icon: FileText, color: "#D97706" },
  { id: "goal" as const, label: "Meta", icon: Target, color: "#7C3AED" },
  { id: "project" as const, label: "Projeto", icon: FolderKanban, color: "#6366F1" },
];
...
export default function QuickAddModal({ onClose, onSuccess, initialType = null }: Props) {
  ...
  const submit = (event: FormEvent) => {
    ...
    setData(current => {
      if (type === "task") return { ...current, tasks: [...current.tasks, { id: nextId(current.tasks), title: title.trim(), done: false, priority: "normal", category, date, project: null }] };
      if (type === "event") return { ...current, events: [...current.events, { id: nextId(current.events), title: title.trim(), date, time, endTime, category, local: description.trim() || "Não informado" }] };
      if (type === "note") return { ...current, notes: [{ id: nextId(current.notes), title: title.trim(), content: description, category, tags: [], favorite: false, updated: today() }, ...current.notes] };
        ...
        {!type ? (
          <div className="p-4 grid grid-cols-2 gap-2">
            {options.map(option => {
              ...
              return <button key={option.id} onClick={() => setType(option.id)} className="flex items-center gap-3 px-4 py-3 rounded-lg border border-[var(--border)] hover:border-[var(--primary)] hover:bg-[var(--accent)] text-left transition-all">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${option.color}15` }}><Icon size={16} style={{ color: option.color }} /></div>
          ...
          <form onSubmit={submit} className="p-5 space-y-3">
            <label className="field-label">Título<input autoFocus required value={title} onChange={event => setTitle(event.target.value)} className="field-input" placeholder={`Nome da ${labels[type].toLowerCase()}`} /></label>
            {(type === "note" || type === "goal" || type === "project") && <label className="field-label">Descrição<textarea value={description} onChange={event => setDescription(event.target.value)} className="field-input min-h-24 resize-y" placeholder="Detalhes..." /></label>}
            {type === "event" && <label className="field-label">Local<input value={description} onChange={event => setDescription(event.target.value)} className="field-input" placeholder="Local ou link" /></label>}

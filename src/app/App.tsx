...
import QuickAddModal, {
  type AddType,
} from "@/presentation/components/QuickAddModal"
...
export default function App() {
  ...
  useEffect(() => {
    const handler = (event: Event) =>
      setLegacyData((event as CustomEvent<string>).detail)
  ...
  useEffect(() => {
    ...
    api
      .get<{ title: string; route: string }[]>("/api/v1/notifications")
  ...
  if (loading)
    return (
      <div className="min-h-screen grid place-items-center text-sm text-[var(--muted-foreground)]">
        Carregando seus dados...
      </div>
    )
  if (error && !appData.user.email)
    return (
  ...
  return (
    ...
    >
          ...
          <div className="absolute z-40 top-12 right-24 w-80 bg-white border border-[var(--border)] rounded-xl shadow-xl p-2">
            ...
            {notifications.map((item) => (
              <button
                ...
                className="w-full text-left px-3 py-2.5 rounded-lg hover:bg-[var(--secondary)] text-[12.5px]"
              >
                <span className="inline-block w-1.5 h-1.5 bg-[var(--primary)] rounded-full mr-2" />
                {item.title}
              </button>
            ))}
            {notifications.length === 0 && (
              <p className="px-3 py-5 text-center text-[12px] text-[var(--muted-foreground)]">
      ...
      {saveStatus === "error" && (
        <div
          role="alert"
          className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 bg-red-700 text-white px-4 py-2 rounded-lg text-sm"
        >
      ...
      {legacyData && (
        <div className="fixed inset-0 z-[60] bg-black/40 grid place-items-center p-4">
          ...
          >
            <h2 id="legacy-title" className="text-lg font-semibold">
              Dados locais encontrados
            </h2>
            <p className="text-sm text-[var(--muted-foreground)] mt-2">
              Encontramos dados salvos nesta versão do Life OS. Deseja
              importá-los para sua conta?
            </p>
            <div className="mt-5 flex justify-end gap-2">
              <button
                onClick={() => setLegacyData(null)}
              ...
              <button
                onClick={async () => {
                  try {
                    await importData(
                      new File([legacyData], "legacy.json", {
                        type: "application/json",
                      }),
                    )
                    setLegacyData(null)
                    setToast(
                      "Dados locais enviados para sua conta. Você pode remover o backup em Configurações.",
                    )
                  } catch {
                    setToast("O backup local não pôde ser validado.")
                  }
                }}

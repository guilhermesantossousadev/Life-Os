...
export default function Tasks() {
  ...
  useEffect(() => {
    ...
    if (task) {
      setView(task.done ? "concluidas" : "todas")
      setEditing(task.id)
    }
  ...
  const views: { id: View; label: string; count: number }[] = [
    {
      id: "hoje",
      label: "Hoje",
      count: tasks.filter((t) => t.date === todayStr && !t.done).length,
    },
    {
      id: "proximas",
      label: "Próximas",
      ...
      {showAdd && (
        <div className="bg-white rounded-xl border border-[var(--primary)]/30 p-4 mb-4 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={addTask}
              className="px-3 py-1.5 bg-[var(--primary)] text-white text-[12px] rounded-lg hover:bg-blue-700"
            >
              Adicionar
            </button>
            <button
              onClick={() => setShowAdd(false)}
              className="px-3 py-1.5 text-[12px] text-[var(--muted-foreground)] hover:bg-[var(--secondary)] rounded-lg"
            >
              Cancelar
            </button>
          </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-[13px] text-[var(--muted-foreground)] mt-1">
            Aproveite ou adicione uma nova.
          </p>
        </div>
      ) : (
        <div className="space-y-1.5">
          {filtered.map((task) => (
            ...
            >
              <div className="flex items-start gap-3 px-4 py-3.5">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-1 text-[var(--muted-foreground)] hover:text-[var(--primary)] focus:opacity-100"
                  >
                    <Pencil size={12} />
                  </button>
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-1 text-[var(--muted-foreground)] hover:text-red-500 focus:opacity-100"
                  >
                    <Trash2 size={12} />
                  </button>
                </div>
              ...
              {editing === task.id && (
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-2 px-4 pb-4 border-t border-[var(--border)] pt-3">
                  <label className="sm:col-span-4 field-label">
                    Título
                    <input
                      value={task.title}
                      onChange={(event) =>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setData((current) => ({
                          ...
                          tasks: current.tasks.map((item) =>
                            item.id === task.id
                              ? { ...item, project: event.target.value || null }
                              : item,
                          ),
                        }))
                      }
                  ...
                  <div className="sm:col-span-4">
                    <p className="field-label mb-2">Subtarefas</p>

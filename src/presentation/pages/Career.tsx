...
import CareerManager from "@/presentation/components/CareerManager"
...
const statusIcon = (s: string) => {
  if (s === "atual")
    return (
      <div className="w-3 h-3 rounded-full bg-[var(--primary)] ring-4 ring-[var(--accent)]" />
    )
  if (s === "próximo")
    return (
  ...
  return (
    <div className="w-3 h-3 rounded-full bg-[var(--border)] ring-4 ring-[var(--secondary)]" />
  )
}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-5xl">
        ...
        <div className="bg-white rounded-xl border border-[var(--border)] p-5">
          ...
          <div className="space-y-3">
            ...
            <div className="pt-3 border-t border-[var(--border)] space-y-2">
              {[
                {
                  label: "Desde",
                  value: formatCivilDate(career.current.start, {
                    month: "long",
                    year: "numeric",
                  }),
                },
                ...
                {
                  label: "Salário",
                  value: career.current.salary.toLocaleString("pt-BR", {
                    style: "currency",
                    currency: "BRL",
                  }),
                },
          ...
          <div className="mt-5 pt-4 border-t border-[var(--border)]">
            <p className="text-[10.5px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)] mb-2">
        ...
        {/* Career path */}
        <div className="bg-white rounded-xl border border-[var(--border)] p-5">
          <h2 className="text-[13px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)] mb-5">
            Trajetória
          </h2>
        ...
        {/* Skills */}
        <div className="bg-white rounded-xl border border-[var(--border)] p-5">
          <h2 className="text-[13px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)] mb-4">
            Competências
          </h2>
          <div className="space-y-2 mb-6">
            {career.skills.map((skill) => {
              ...
              return (
                <div
                  key={skill.name}
                  className="flex items-center justify-between px-3 py-2 rounded-lg bg-[var(--secondary)]"
                >
          ...
          <h2 className="text-[13px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)] mb-3">
            Certificações
          </h2>
          <div className="space-y-2">
            {career.certifications.map((cert) => (
              <div
                key={cert.name}
                className="flex items-start gap-3 p-3 rounded-lg bg-[var(--secondary)]"
              >
        ...
        <div className="lg:col-span-3 bg-white rounded-xl border border-[var(--border)] p-5">
          ...
          <div className="space-y-3">
            {[
              career.current,
              ...career.history.map((h) => ({ ...h, current: false })),
            ].map((job, i) => (
              <div
                key={i}
                className="flex items-start gap-4 p-4 rounded-xl bg-[var(--secondary)]"
              >
                <div
                ...
                >
                  <Briefcase
                    size={15}
                    className={
                      i === 0 ? "text-white" : "text-[var(--muted-foreground)]"
                    }
                  />
                </div>
                ...
                <div className="text-right text-[12px] text-[var(--muted-foreground)]">
                  <p>
                    {formatCivilDate((job as any).start, {
                      month: "short",
                      year: "numeric",
                    })}
                  </p>
                  <p>
                    {(job as any).end
                      ? formatCivilDate((job as any).end, {
                          month: "short",
                          year: "numeric",
                        })
                      : "Atual"}
                ...
                {i === 0 && (
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-[var(--primary)] bg-[var(--accent)] px-2 py-0.5 rounded-full self-start flex-shrink-0">
                    Atual
                  </span>
                )}
              </div>

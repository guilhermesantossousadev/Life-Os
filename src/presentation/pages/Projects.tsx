...
export default function Projects() {
  ...
  useEffect(() => {
    ...
    const project = projects.find((item) => item.serverId === id)
            ...
            >
              ...
              <div className="mb-3">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-[11px] text-[var(--muted-foreground)]">
                    {doneTasks}/{p.tasks.length} tarefas
                  </span>
                  <span
              ...
              <div className="flex gap-1.5 flex-wrap">
                {p.tags.map((tag) => (
                  <span
                    key={tag}
                    className="flex items-center gap-1 text-[10.5px] px-2 py-0.5 rounded-full bg-[var(--secondary)] text-[var(--muted-foreground)]"
                  >
                    <Tag size={9} />
                    {tag}
                  </span>
          ...
          <div className="p-5">
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              {[
                ...
                {
                  label: "Prazo",
                  value: formatCivilDate(selectedProject.deadline),
                },
                { label: "Progresso", value: `${selectedProject.progress}%` },
                {
                  label: "Tarefas",
                  value: `${selectedProject.tasks.filter((t) => t.done).length}/${selectedProject.tasks.length}`,
                },
              ].map((item) => (
                <div
                  key={item.label}
                  className="p-3 rounded-xl bg-[var(--secondary)]"
                >
            ...
            <div className="mb-5">
              ...
              <div className="flex gap-2 mt-3">
                <input
                  ...
                  onKeyDown={(event) =>
                    event.key === "Enter" && addProjectTask()
                  }
                ...
                <button
                  ...
                  className="px-3 bg-[var(--primary)] text-white rounded-lg text-[12px]"
                >
                  Adicionar
                </button>
              </div>
            ...
            <div>
              ...
              <div className="flex gap-1.5 flex-wrap">
                {selectedProject.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-[11px] px-2.5 py-1 rounded-full bg-[var(--accent)] text-[var(--accent-foreground)] font-medium"
                  >
                    {tag}{" "}
                    {selectedProject.serverId &&
                      selectedProject.tagIds?.[tag] && (
                        <button
                          aria-label={`Remover tag ${tag}`}
                          onClick={async () => {
                            await api.delete(
                              `/api/v1/projects/${selectedProject.serverId}/tags/${selectedProject.tagIds?.[tag]}`,
                            )
                            await reload()
                          }}
                        >
                ...
                {selectedProject.serverId && (
                  <form
                    onSubmit={async (event) => {
                      ...
                      await api.post(
                        `/api/v1/projects/${selectedProject.serverId}/tags/${tag.id}`,
                      )
                      setNewTag("")
                      await reload()
                    }}
                  ...
                  >
                    <input
                      ...
                      className="w-20 text-xs border-b bg-transparent"
                    />
                    <button className="text-xs text-[var(--primary)]">+</button>
                  </form>
                )}
              </div>
            </div>

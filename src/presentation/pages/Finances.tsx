...
import { today } from "@/application/state/DataContext"
import FinanceManager from "@/presentation/components/FinanceManager"
...
export default function Finances() {
  ...
  const monthIncome = finances.transactions
    .filter((t) => t.type === "receita" && t.date.startsWith(currentMonth))
              ...
              {finances.transactions.slice(0, 5).map((t) => (
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      color: t.type === "receita" ? "#059669" : "#EF4444",
                      fontFamily: "var(--font-mono-family)",
                    }}
                  >
                    {t.type === "receita" ? "+" : "-"}
        ...
        <div className="max-w-3xl bg-white rounded-xl border border-[var(--border)] overflow-hidden">
          ...
          <div className="divide-y divide-[var(--border)]">
            {finances.transactions.map((t) => (
              <div
                key={t.id}
                className="group flex items-center gap-3 px-4 py-3 hover:bg-[var(--secondary)] transition-colors"
              >
                ...
                <button
                  ...
                  onClick={() =>
                    setData((current) => ({
                      ...
                      finances: {
                        ...current.finances,
                        transactions: current.finances.transactions.filter(
                          (item) => item.id !== t.id,
                        ),
                      },
                    }))
                  }
                  className="opacity-0 group-hover:opacity-100 text-[var(--muted-foreground)] hover:text-red-500"
                >
                  <Trash2 size={13} />
                </button>
              </div>
          ...
          {finances.accounts.map((acc) => (
            ...
            >
              ...
              <label className="text-[10px] text-[var(--muted-foreground)]">
                Saldo inicial
                <input
                  aria-label={`Saldo inicial de ${acc.name}`}
                  type="number"
                  ...
                  onChange={(event) =>
                    setData((current) => ({
                      ...
                      finances: {
                        ...current.finances,
                        accounts: current.finances.accounts.map((item) =>
                          item.id === acc.id
                            ? {
                                ...item,
                                ...
                                balance:
                                  ...
                                  (item.initialBalance ?? item.balance),
                              }
                            : item,
                        ),
                      },
                  ...
                  className="mt-1 w-full bg-transparent text-[22px] font-bold"
                  style={{
                    color: acc.color,
                    fontFamily: "var(--font-mono-family)",
                  }}
                ...
                <div>
                  ...
                  <div className="h-1.5 bg-[var(--secondary)] rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${usedPct}%`, background: card.color }}
                    />
                  </div>
                </div>
              </div>
        ...
        <div className="max-w-2xl space-y-3">
          {finances.installments.map((inst) => {
            ...
            const pct = Math.round(
              (inst.current / inst.total_installments) * 100,
            )
            return (
              <div
                key={inst.id}
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[12px] text-[var(--muted-foreground)] flex-shrink-0">
                    {inst.current}/{inst.total_installments} · Faltam{" "}
                    {fmt(remaining * inst.installment)}
                  </span>
                </div>
                {inst.serverId && (
                  <button
                    onClick={async () => {
                      await api.post(
                        `/api/v1/finances/installment-purchases/${inst.serverId}/settle`,
                      )
                      await reload()
                    }}
                    className="mt-3 text-xs text-[var(--primary)]"
                  >
        ...
        <div className="max-w-2xl space-y-3">
          {finances.debts.map((d) => {
            const paidPct = Math.round(
              ((d.initial - d.remaining) / d.initial) * 100,
            )
            return (
              <div
                key={d.id}
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-3 text-center mb-4">
                  {[
                    ...
                    >
                      <p className="text-[10px] text-[var(--muted-foreground)] mb-0.5">
                        {item.label}
                      </p>
                      <p
                ...
                <div>
                  <div className="flex justify-between mb-1 text-[11px] text-[var(--muted-foreground)]">
                    ...
                    <span>
                      {fmt(d.initial - d.remaining)} de {fmt(d.initial)}
                    </span>
                  </div>
                ...
                {d.serverId && data.finances.accounts[0]?.serverId && (
                  <button
                    onClick={async () => {
                      await api.post(
                        `/api/v1/finances/debts/${d.serverId}/payments`,
                        {
                          amount: Math.min(d.installment, d.remaining),
                          accountId: data.finances.accounts[0].serverId,
                          date: today(),
                        },
                      )
                      await reload()
                  ...
                  >
                    ...
                    {fmt(Math.min(d.installment, d.remaining))}
                  </button>
                )}
              </div>
            )
                ...
                <div key={b.category} className="px-5 py-4">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setData((current) => ({
                            ...
                            finances: {
                              ...current.finances,
                              budgets: current.finances.budgets.map((item) =>
                        ...
                        className="w-24 text-right bg-transparent text-[var(--muted-foreground)]"
                      />
                    </div>
                  </div>
                  <div className="h-2 bg-[var(--secondary)] rounded-full overflow-hidden">
                  ...
                  {over && (
                    <p className="text-[11px] text-red-500 mt-1">
                      Acima do limite em {fmt(b.spent - b.limit)}
                    </p>
                  )}
                </div>
              )

...
import { APP_TIME_ZONE, todayInSaoPaulo } from "@/shared/datetime/dates"
...
export default function Dashboard({
  ...
  const monthIncome = data.finances.transactions
    ...
    >
      ...
      <div className="mb-8">
        <h1 className="text-[28px] font-semibold text-[var(--foreground)]">
          {greeting}, {data.user.name || "você"}
        </h1>
        <p className="text-[var(--muted-foreground)] text-[14px] mt-0.5">
          {new Intl.DateTimeFormat("pt-BR", {
            timeZone: APP_TIME_ZONE,
            weekday: "long",
            day: "numeric",
      ...
      {highPri > 0 && (
        <div className="mb-6 flex items-center gap-3 px-4 py-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-[13.5px]">
          ...
          <span>
            Você tem{" "}
            <strong>
              {highPri} tarefa{highPri > 1 ? "s" : ""} de alta prioridade
            </strong>{" "}
            para hoje.
          </span>
        </div>
        ...
        {[
          {
            ...
            icon: CheckSquare,
            color: "#2563EB",
            sub: `${tasks.filter((t) => t.done).length} concluídas`,
          },
          {
            label: "Compromissos",
            value: `${todayEvents.length} hoje`,
            icon: Calendar,
            color: "#059669",
            sub: todayEvents[0]?.time + " " + (todayEvents[0]?.title ?? "—"),
          },
          {
            label: "Saldo total",
            ...
            icon: Wallet,
            color: "#7C3AED",
            sub: `Gasto: ${fmt(monthExpense)} este mês`,
          },
          {
            label: "Faculdade",
            value: nextAssignment
              ? `${nextAssignment.title.slice(0, 22)}${
                  nextAssignment.title.length > 22 ? "…" : ""
                }`
              : "Sem entregas",
            icon: BookOpen,
            color: "#D97706",
            sub: nextAssignment
              ? "Entrega: " + nextAssignment.due
              : "Tudo em dia",
          },
        ].map((card) => {
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {task.priority === "alta" && !task.done && (
                      <span className="text-[10px] font-semibold uppercase tracking-wide text-red-600 bg-red-50 border border-red-100 px-1.5 py-0.5 rounded">
                        Alta
                      </span>
                    )}
                  </div>
        ...
        <div className="bg-white rounded-xl border border-[var(--border)] p-5">
          ...
          <div className="space-y-3">
            ...
            <div className="border-t border-[var(--border)] pt-3 flex justify-between items-center">
              ...
              <span
                className="text-[14px] font-bold text-[var(--foreground)]"
                style={{ fontFamily: "var(--font-mono-family)" }}
              >
                {fmt(totalBalance)}
              </span>
            </div>
            {data.finances.installments.slice(0, 2).map((inst) => (
              <div
                key={inst.id}
                className="flex justify-between items-center bg-[var(--secondary)] rounded-lg px-3 py-2"
              >
                <div>
                  <p className="text-[12px] text-[var(--foreground)]">
                    {inst.desc.slice(0, 20)}
                  </p>
                  <p className="text-[10px] text-[var(--muted-foreground)]">
                    {inst.current}/{inst.total_installments} parcelas
                  </p>
                </div>
                <span

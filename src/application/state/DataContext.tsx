...
import type { ApiClient, ResourceGateway, ServerEntity } from "@/application/ports/api";
import type { AppData, Asset, CalendarEvent, DocumentItem, Goal, InboxItem, Note, Project, Task } from "@/domain/entities";
import { civilDate, localTime, saoPauloLocalToIso, todayInSaoPaulo } from "@/shared/datetime/dates";
import { useAuth } from "@/application/state/AuthContext";

export type { AppData, Asset, CalendarEvent, DocumentItem, Goal, InboxItem, Note, Project, Task } from "@/domain/entities";
...
export function DataProvider({ children, apiClient, resourceGateway }: { children: ReactNode; apiClient: ApiClient; resourceGateway: ResourceGateway }) {
  const { user } = useAuth();
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (workspace.profile?.avatarUrl) {
        try { mapped.user.avatar = (await apiClient.get<{ url: string }>("/api/v1/profile/avatar-url")).url; } catch { /* initials remain available */ }
      }
  ...
  const syncCollection = useCallback(async <T extends { id: number; serverId?: string }>(section: string, path: string, before: T[], after: T[], mapper: (item: T) => unknown) => {
    for (const oldItem of before) if (oldItem.serverId && !after.some(item => item.id === oldItem.id)) await resourceGateway.remove(path, oldItem.serverId);
    for (const item of after) {
      const oldItem = before.find(candidate => candidate.id === item.id);
      if (!oldItem || !item.serverId) {
        const created = await resourceGateway.create<ServerEntity>(path, mapper(item));
        attachServerId(section, item.id, created.id);
      } else if (JSON.stringify(oldItem) !== JSON.stringify(item)) await resourceGateway.update(path, item.serverId, mapper(item));
    }
  ...
  const sync = useCallback(async (before: AppData, after: AppData) => {
    ...
    for (const name of addedCategories) { const created = await resourceGateway.create<ServerEntity>("categories", { name, domain: "general" }); categoryIds.current.set(name, created.id); }
    ...
    for (const task of after.tasks.filter(item => item.serverId)) {
      ...
      for (const previous of previousSubtasks) if (previous.serverId && !currentSubtasks.some(item => item.id === previous.id)) await resourceGateway.remove("subtasks", previous.serverId);
      for (let position = 0; position < currentSubtasks.length; position++) {
        ...
        if (!previous || !subtask.serverId) { const created = await resourceGateway.create<ServerEntity>("subtasks", payload); attachSubtaskServerId(task.id, subtask.id, created.id); }
        else if (JSON.stringify(previous) !== JSON.stringify(subtask) || previousSubtasks.indexOf(previous) !== position) await resourceGateway.update("subtasks", subtask.serverId, payload);
      }
    }
    ...
    for (const goal of after.goals.filter(item => item.serverId)) {
      ...
      for (const previous of previousActions) if (previous.serverId && !currentActions.some(item => item.id === previous.id)) await resourceGateway.remove("goal-actions", previous.serverId);
      for (let position = 0; position < currentActions.length; position++) { const action = currentActions[position]; const previous = previousActions.find(item => item.id === action.id); const payload = { goalId: goal.serverId, title: action.title, isCompleted: action.done, position }; if (!previous || !action.serverId) { const created = await resourceGateway.create<ServerEntity>("goal-actions", payload); attachGoalActionServerId(goal.id, action.id, created.id); } else if (JSON.stringify(previous) !== JSON.stringify(action)) await resourceGateway.update("goal-actions", action.serverId, payload); }
    }
    ...
    for (const project of after.projects.filter(item => item.serverId)) {
      ...
      for (const previousTask of previousTasks) {
        ...
        if (serverId && !project.tasks.some(item => item.id === previousTask.id)) await resourceGateway.remove("tasks", serverId);
      }
      for (const task of project.tasks) {
        ...
        if (!previousTask || !serverId) { const created = await resourceGateway.create<ServerEntity>("tasks", payload); attachProjectTaskServerId(project.id, task.id, created.id); }
        else if (JSON.stringify(previousTask) !== JSON.stringify(task)) await resourceGateway.update("tasks", serverId, payload);
      }
    }
    ...
    if (before.user.name !== after.user.name) await apiClient.patch("/api/v1/profile", { name: after.user.name });
    if (before.theme !== after.theme || JSON.stringify(before.preferences) !== JSON.stringify(after.preferences)) await apiClient.put("/api/v1/preferences", { theme: ({ claro: "light", escuro: "dark", sistema: "system" } as const)[after.theme], dateFormat: "dd/MM/yyyy", firstPage: "dashboard", timeZone: "America/Sao_Paulo", taskDueNotifications: after.preferences["Tarefas com prazo hoje"] ?? true, financeDueNotifications: after.preferences["Contas próximas do vencimento"] ?? true, studyDueNotifications: after.preferences["Atividades da faculdade"] ?? true, assetMaintenanceNotifications: after.preferences["Próximas manutenções do veículo"] ?? true, weeklySummary: after.preferences["Resumo semanal"] ?? true });

    const removedCategories = before.categories.filter(name => !after.categories.includes(name));
    for (const name of removedCategories) { const id = categoryIds.current.get(name); if (id) { await resourceGateway.remove("categories", id); categoryIds.current.delete(name); } }
  }, [apiClient, attachGoalActionServerId, attachProjectTaskServerId, attachSubtaskServerId, categoryId, resourceGateway, syncCollection]);
...
function messageFrom(reason: unknown) {
  ...
  const traceId = (reason as Error & { problem?: { traceId?: string } }).problem?.traceId;

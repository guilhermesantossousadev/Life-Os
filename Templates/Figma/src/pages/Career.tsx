...
import { mockCareer } from "../data/mock";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-5xl">
        ...
        <div className="bg-white rounded-xl border border-[var(--border)] p-5">
          ...
          <div className="space-y-3">
            <div>
              <p className="text-[18px] font-bold text-[var(--foreground)]">{mockCareer.current.role}</p>
              <p className="text-[13px] text-[var(--muted-foreground)] mt-0.5">{mockCareer.current.company}</p>
            </div>
            <div className="pt-3 border-t border-[var(--border)] space-y-2">
              {[
                { label: "Desde", value: new Date(mockCareer.current.start).toLocaleDateString("pt-BR", { month: "long", year: "numeric" }) },
                { label: "Local", value: mockCareer.current.location },
                { label: "Salário", value: mockCareer.current.salary.toLocaleString("pt-BR", { style: "currency", currency: "BRL" }) },
              ].map(item => (
          ...
          <div className="mt-5 pt-4 border-t border-[var(--border)]">
            ...
            <p className="text-[13px] text-[var(--foreground)]">{mockCareer.objective}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-[var(--border)] p-5">
          ...
          <div className="space-y-4">
            {mockCareer.path.map((step, i) => (
              <div key={i} className="flex items-center gap-4 relative">
                {i < mockCareer.path.length - 1 && (
        ...
        <div className="bg-white rounded-xl border border-[var(--border)] p-5">
          ...
          <div className="space-y-2">
            {mockCareer.certifications.map(cert => (
        ...
        <div className="lg:col-span-3 bg-white rounded-xl border border-[var(--border)] p-5">
          ...
          <div className="space-y-3">
            {[mockCareer.current, ...mockCareer.history.map(h => ({ ...h, current: false }))].map((job, i) => (

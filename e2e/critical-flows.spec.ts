...
test.beforeEach(() => {
  test.skip(!email || !password, "Defina E2E_EMAIL e E2E_PASSWORD para executar contra o ambiente Supabase de testes.");
});
...
async function expectSuccessfulWrite(page: Page, path: RegExp, action: () => Promise<unknown>) {
  const responsePromise = page.waitForResponse(
    response => path.test(new URL(response.url()).pathname) && ["POST", "PUT", "PATCH", "DELETE"].includes(response.request().method()),
    { timeout: 30_000 },
  );
  await action();
  const response = await responsePromise;
  expect(response.ok(), `${response.request().method()} ${response.url()} retornou ${response.status()}`).toBeTruthy();
}

test("cadastro/login e dashboard", async ({ page }) => {
  await login(page);
  await expect(page.getByText(/visão geral|tarefas de hoje/i).first()).toBeVisible();
});
...
test("criar, editar e concluir tarefa", async ({ page }) => {
  ...
  await expectSuccessfulWrite(page, /\/api\/v1\/tasks$/, () => quickAdd.getByRole("button", { name: "Adicionar", exact: true }).click());

  let checkbox = page.getByRole("checkbox", { name: `Concluir tarefa ${title}` });
  await expect(checkbox).toBeVisible();
  await checkbox.locator("..").getByRole("button", { name: "Editar tarefa" }).click();
...
test("criar transação e refletir no financeiro", async ({ page }) => {
  ...
  await expectSuccessfulWrite(page, /\/api\/v1\/finances\/accounts$/, () => manager.getByRole("button", { name: "Salvar" }).click());

  const expenseTitle = `Despesa E2E ${Date.now()}`;
  await page.getByRole("button", { name: "Registrar" }).click();
  const expenseDialog = page.getByRole("dialog");
  await expect(expenseDialog.getByRole("heading", { name: "Nova despesa" })).toBeVisible();
  ...
  await expectSuccessfulWrite(page, /\/api\/v1\/finances\/transactions$/, () => expenseDialog.getByRole("button", { name: "Salvar" }).click());
...
test("criar evento e visualizar agenda", async ({ page }) => {
  ...
  await expectSuccessfulWrite(page, /\/api\/v1\/events$/, () => dialog.getByRole("button", { name: "Salvar" }).click());
...
test("criar meta e atualizar progresso", async ({ page }) => {
  ...
  await expectSuccessfulWrite(page, /\/api\/v1\/goals$/, () => dialog.getByRole("button", { name: "Salvar" }).click());
  await expect(page.getByText(title)).toBeVisible();
});
...
test("upload, download e exclusão de documento", async ({ page }) => {
  ...
  await expectSuccessfulWrite(page, /\/api\/v1\/documents$/, () => page.getByLabel("Selecionar documentos").setInputFiles({
    name: filename,
    mimeType: "text/plain",
    buffer: Buffer.from("Life OS"),
  }));
  await expect(page.getByRole("status")).toHaveText("Upload concluído.");
  await expect(page.getByText(filename.replace(/\.txt$/, ""), { exact: true })).toBeVisible();

  await expectSuccessfulWrite(page, /\/api\/v1\/documents\//, () => page.getByRole("button", { name: new RegExp(`excluir ${filename.replace(/\.txt$/, "")}`, "i") }).click());

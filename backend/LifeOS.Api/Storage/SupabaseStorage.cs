...
namespace LifeOS.Api.Storage;

public interface IPrivateFileStorage
{
    Task UploadAsync(string path, Stream content, string mimeType, CancellationToken ct);
    Task<string> CreateSignedUrlAsync(string path, TimeSpan validity, CancellationToken ct);
